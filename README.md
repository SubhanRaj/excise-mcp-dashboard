...
3. The model writes a Python plot script (pandas, NumPy, Matplotlib, Plotly).
   It runs in a bubblewrap sandbox with no network, a read-only filesystem, one
   writable scratch directory, and wall clock and memory caps.
...
Charts are generated by a Python engine (pandas, NumPy, SciPy, Matplotlib,
Plotly). The engine sits behind an `IVisualizationEngine` adapter, so
[GNU Octave](https://octave.org),
[MATLAB](https://in.mathworks.com/products/matlab.html), or
[Wolfram Mathematica](https://www.wolfram.com/mathematica/) can be added later
against the same interface. The first build ships Python only —
[MCP_ENGINES.md](MCP_ENGINES.md) and [EVALUATION.md](EVALUATION.md)
§Right-sizing cover why.
