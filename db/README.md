...
| `schema.sql` | `citext` extension; the base tables (dimensions, fact tables, the shops split, `dispatches` / `dispatch_strength_lines`, reference tables); the `etl` bookkeeping tables; the `kb` tables. One `set_updated_at()` trigger function with a `BEFORE UPDATE` trigger on every table that has an `updated_at` column. The fact-table and lookup indexes. |
...
| `seed_reference.sql` | `zones` (5), `divisions` (18), `districts` (all 75, mapped to division and zone), `financial_years` (FY2014-15 through FY2026-27), `license_categories` (the 5 broad kinds plus the specific retail/wholesale codes the IESCMS dispatch import needs — FL2, CL2, FL5DB, FL4A, FL4C, CL5C, CL5CC). Every `INSERT` is `ON CONFLICT DO NOTHING` on the natural key; a re-run is a no-op. |
