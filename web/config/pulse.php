...
return [
    ...
    */
    ...
    // Pulse's own default 'viewPulse' gate is `fn ($user = null) =>
    // $app->environment('local')` — true on this box, which is also genuinely
    // public through the Cloudflare Tunnel. IsAdmin (before Authorize::class,
    // which still runs on top) enforces admin-only access without depending on
    // Gate::define() override-ordering against the package's own registration.
    'middleware' => [
        'web',
        'auth',
        IsAdmin::class,
