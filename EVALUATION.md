...
| Brand assets + GIGW/UX4G chrome — State Emblem of Uttar Pradesh (`up-gov-emblem.svg`, `up-gov-emblem-white.png`), favicons, app icons, OG card, `scripts/make-brand-assets.php` generator, the "Government of Uttar Pradesh" identity strip, A-/A/A+ text-size and high-contrast toggles, skip-to-content link, footer policy links | `upexcise-stats-dashboard/public/assets/img/*`, `public/{favicon*,icon-*,apple-touch-icon}.png`, `scripts/make-brand-assets.php`, the public layout Blade | Copy the asset files into `web/public/assets/img/` and `web/public/`; port the identity strip, accessibility toggles and footer into the authed layout. `og-default.png` regenerates from the generator |
