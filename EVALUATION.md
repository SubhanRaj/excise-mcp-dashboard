...
| Brand assets + GIGW/UX4G chrome — State Emblem of Uttar Pradesh, favicons, app icons, `scripts/make-brand-assets.php` generator, OG card, the "Government of Uttar Pradesh" identity strip, A-/A/A+ text-size and high-contrast toggles, skip-to-content link, footer policy links | `assets/brand/` in this repo (emblem SVG + white PNG, favicons, app icons); the chrome, `scripts/make-brand-assets.php` and `og-default.png` from `~/Sites/upexcise-stats-dashboard`'s public layout | Move `assets/brand/*` into `web/public/` at Milestone 5; port the identity strip, accessibility toggles and footer into the authed layout; regenerate the icons and `og-default.png` with the sibling's generator |
